...
equal(save_settings.slots.slot_3[2].id, 914)
equal(save_settings.slots.slot_3[3].id, 951)
...
equal(save_settings.slots.slot_3[3].name, 'Rahal')
rahal.en = 'Rahal'

-- Staged additions and dismissals never leak into a saved preset. Saving is a
-- snapshot of the confirmed party only.
local confirmed_only_settings = presets.new_settings()
local confirmed_only_ok = presets.save(confirmed_only_settings, {
    active = {{trust=valaineral, identity_key='valaineral'}},
    pending = {valaineral, mihli},
    dismissals = {valaineral=true},
})
assert(confirmed_only_ok)
equal(#confirmed_only_settings.slots.slot_1, 1)
equal(confirmed_only_settings.slots.slot_1[1].id, 910)
