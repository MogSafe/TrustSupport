...
-- Save the intended party after the current editing plan is applied. Retained
-- active Trusts keep their authoritative party order, followed by pending
-- summons in selection order. The operation is atomic; malformed retained or
-- pending data never replaces the previous slot contents.
function presets.save(settings, context, slot)
    ...
    local dismissals = context.dismissals or {}
    if type(active) ~= 'table' or type(pending) ~= 'table'
        or type(dismissals) ~= 'table' then
        return false, 'invalid_context'
    end
    ...
    for _, value in ipairs(active) do
        local trust = type(value) == 'table' and value.trust or nil
        local identity_key = type(value) == 'table'
            and (value.identity_key
                or (type(trust) == 'table' and trust.identity_key)) or nil
        if not (identity_key and dismissals[identity_key]) then
            local ok, reason, name = append_intended(
                intended, seen_ids, seen_identities, value)
            if not ok then
    ...
    settings.slots[presets.slot_key(index)] = intended
