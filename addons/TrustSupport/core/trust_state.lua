...
function State:unstage_dismissal(query)
    ...
    -- Undo must remain atomic when a staged replacement already consumes the
    -- dismissal's temporary slot. Pairing follows the same authoritative
    -- party order and pending selection order used by the UI. If keeping this
    -- Trust would exceed current capacity, remove its paired summon; otherwise
    -- preserve every pending summon and let the preview rows reflow.
    local outgoing_index = 0
    for _, party_record in ipairs(self.party_trusts) do
        if self.pending_dismissals[party_record.identity_key] then
            outgoing_index = outgoing_index + 1
            if party_record.identity_key == identity_key then
                break
            end
        end
    end
    local intended_count = #self.party_trusts
        - count_entries(self.pending_dismissals) + #self.pending
    local capacity = math.max(0, self.max_trusts - self.other_members)
    local removed_entry = nil
    if intended_count + 1 > capacity and #self.pending > 0 then
        local removal_index = outgoing_index >= 1
            and outgoing_index <= #self.pending and outgoing_index
            or #self.pending
        local removed_id = table.remove(self.pending, removal_index)
        removed_entry = self.by_id[removed_id]
    end
    self.pending_dismissals[identity_key] = nil
    return true, 'dismiss_removed', record, removed_entry
end
