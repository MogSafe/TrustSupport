...
function Handler:keep(args)
    ...
    local ok, reason, record, removed =
        self.state:unstage_dismissal(join(args, 2))
    if not ok then
        self:_failure(reason)
        return {ok=false, reason=reason, record=record}
    end
    local kept_name = record and record.trust and record.trust.en
        or record and record.name or 'Trust'
    if removed then
        self.emit(('%s will remain in the party; %s was removed from pending summons to stay within capacity.')
            :format(kept_name, removed.en or removed.name or 'a Trust'))
    else
        self.emit(('%s will remain in the party.'):format(kept_name))
    end
    return {ok=true, record=record, removed=removed}
end
